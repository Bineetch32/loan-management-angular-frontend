import {Injectable} from '@angular/core';import {HttpClient} from '@angular/common/http';import {Loan,EmiResponse} from './models';
@Injectable({providedIn:'root'})
export class LoanService{private baseUrl='http://localhost:8081';constructor(private http:HttpClient){}
getLoans(){return this.http.get<Loan[]>(this.baseUrl+'/api/loans')}getLoan(id:number){return this.http.get<Loan>(this.baseUrl+'/api/loans/'+id)}
...
emi(data:{loanAmount:number;annualInterestRate:number;tenureInYears:number}){return this.http.post<EmiResponse>(this.baseUrl+'/api/emi',data)}}